...
A tasty bite of SQLite.

sql-hummus has simple defaults for using SQLite files as timestamped logs or key-value stores.
...
sql-hummus log push log.db "Writing a new element to a file that may exist"

# Append multiple lines from stdin
echo "This will all\nbe one element." | sql-hummus log push log.db

# Read back the elements as JSONL
sql-hummus log iter log.db | jq .

# Get a single entry by its ID
sql-hummus log get log.db 0

# Slice into the log file using a half-open interval
sql-hummus log get log.db 0..2

# Slice into the log using a date interval
sql-hummus log get log.db "2026-07-07 04:31:00+00:00..2026-07-07 04:32:00+00:00"
```
...
sql-hummus kv insert my-settings.db "/myapp/color_mode" "light_mode"

# Upsert the same element
sql-hummus kv insert my-settings.db "/myapp/color_mode" "dark_mode"

# Return the most-recently-inserted value for the given key
sql-hummus kv get my-settings.db "/myapp/color_mode"

# Returns non-zero if the key isn't present
sql-hummus kv contains-key my-settings.db "/myapp/color_mode"

# Insert an empty string to use any KV file as a set
sql-hummus kv insert my-settings.db "/myapp/bookmarks/https://example.com/" ""

# Get a range of elements matching a prefix
sql-hummus kv with-prefix my-settings.db "/myapp/"

# Get all elements
sql-hummus kv with-prefix my-settings.db
```
